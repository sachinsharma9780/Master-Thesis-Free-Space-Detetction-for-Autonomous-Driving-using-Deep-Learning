...
def train(c2d, c1d, optimizer, data_loader, device, epoch):
    ...
    for data in tqdm(data_loader):
        image, edge_maps, target, road_gt = data['image'].to(device), data['gray_image'] , data['label'].squeeze(1) , data["road_label"].to(device)
...
class berkely_driving_dataset(torch.utils.data.Dataset):
    def __init__(self, path, type='train', edge_type='train_edge_maps' , transform=None, color = True):
    ...
    # it generally has the following diretory structure
        ...
        self.imgs = sorted(glob(os.path.join(self.path, 'images/100k/' + self.type + '/*.jpg')))
        self.grayscale_imgs = sorted(glob(os.path.join(self.path, 'images/100k/' + self.edge_type + '/*.jpg')))
        if color is True:
   ...
   #  to support the indexing such that dataset[i] can be used to get ith sample 
    ...
    def __getitem__(self, idx):
        ...
        gray_img_name = self.grayscale_imgs[idx]
        ...
        if self.type == "train":
            ...
            edge_maps = Image.open(gray_img_name)
            edge_maps = ImageOps.grayscale(edge_maps)
            #grayscale_image = ImageOps.grayscale(image)
            if self.transform:
                image = self.transform(image)
                label = self.transform(label)
                edge_maps = self.transform(edge_maps)
            #print('img', image.shape, 'label', label.shape)
            return {'image':image, 'gray_image':edge_maps, 'label':label, 'road_label':scene_gt}
...
#model_checkpoint = "/home/sachin/Desktop/free_space_detection_script/scripts/experiments/uncertainity/fusion_with_edge_maps/exp7/best_chkpt.pth.tar"
...
bdd_train = berkely_driving_dataset(PATH_TO_BERKELY_DATASET, transform=bdd_transforms,  type='train', edge_type='train_edge_maps', color = False)
bdd_val = berkely_driving_dataset(PATH_TO_BERKELY_DATASET, transform=bdd_transforms,  type='val', edge_type='val_edge_maps', color = False)
...
save_path_prefix = '/home/sachin/Desktop/free_space_detection_script/scripts/experiments/uncertainity/fusion_with_edge_maps/exp6'
